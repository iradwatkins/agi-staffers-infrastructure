#!/bin/bash
# Deploy Emergency Rollback System to AGI Staffers VPS with authentication
...
SSH_CMD="sshpass -p 'Bobby321&Gloria321Watkins?' ssh -o StrictHostKeyChecking=no root@72.60.28.175"
SCP_CMD="sshpass -p 'Bobby321&Gloria321Watkins?' scp -o StrictHostKeyChecking=no"

# Step 1: Create rollback directory structure on VPS
echo "Step 1: Creating rollback infrastructure..."
$SSH_CMD 'mkdir -p /root/rollback-system/{snapshots,configs,scripts,monitoring}'

# Step 2: Upload rollback scripts
echo "Step 2: Uploading rollback scripts..."
$SCP_CMD rollback-scripts/*.sh root@72.60.28.175:/root/rollback-system/scripts/
$SSH_CMD 'chmod +x /root/rollback-system/scripts/*.sh'

# Step 3: Upload blue-green configuration
echo "Step 3: Uploading blue-green deployment configs..."
$SCP_CMD rollback-scripts/docker-compose.blue-green.yml root@72.60.28.175:/root/rollback-system/configs/
$SCP_CMD rollback-scripts/nginx-blue-green.conf root@72.60.28.175:/root/rollback-system/configs/
...
$SSH_CMD '/root/rollback-system/scripts/chicago-timezone-setup.sh'
...
docker-compose -f docker-compose.blue-green.yml up -d admin-dashboard-blue 2>/dev/null || echo "Blue-green setup will be completed during deployment"
EOF
...
$SSH_CMD 'ls -la /root/rollback-system/snapshots/ 2>/dev/null || echo "Snapshots directory created"'

# Check health monitor
echo ""
echo "Health monitor status:"
$SSH_CMD 'systemctl status health-monitor.service --no-pager | head -5'

# Check containers
echo ""
echo "Container status:"
$SSH_CMD 'docker ps | grep -E "admin-dashboard|metrics-api|push-api" || echo "Containers will be started during deployment"'
...
echo ""
echo "Next steps:"
echo "1. Test rollback: $SSH_CMD '/root/rollback-system/scripts/rollback-emergency.sh'"
echo "2. Deploy React dashboard: $SSH_CMD 'docker-compose -f /root/rollback-system/configs/docker-compose.blue-green.yml up -d'"
echo "3. Switch traffic: Update Caddy configuration to route to port 8081 for React"
echo "4. Monitor health: $SSH_CMD 'journalctl -u health-monitor -f'"
echo ""
